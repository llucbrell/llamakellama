...
" Guardar el tamaño de la ventana antes de ejecutar el comando
  ...
  " Usar los valores globales si están definidos o valores por defecto
  let l:endpoint = get(g:, 'ai_endpoint', 'http://localhost:1234/v1/chat/completions')
  let l:model = get(g:, 'ai_model', 'lmstudio-community/gemma-2-2b-it-GGUF')
  let l:system_prompt = get(g:, 'ai_system_prompt', 'Obey literally the orders the user gives you.')
...
function! s:ReadAndUpdateBuffer(timer)
  ...
  for line in l:new_lines
    ...
    if line == '[DONE]'
      ...
      echom "llama: le llama la llama!! Jajajajaj!!!"
      return
    endif
